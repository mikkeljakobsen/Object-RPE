...
def main():
    ...
    if opt.dataset == 'ycb':
        ...
        dataset = PoseDataset_warehouse('train', opt.num_points, True, opt.dataset_root, opt.noise_trans,
                                        opt.refine_start)
    elif opt.dataset == 'linemod':
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        if best_test < opt.refine_margin and not opt.refine_start:
            ...
            if opt.dataset == 'ycb':
                test_dataset = PoseDataset_ycb('test', opt.num_points, False, opt.dataset_root, 0.0, opt.refine_start)
            if opt.dataset == 'warehouse':
                test_dataset = PoseDataset_warehouse('test', opt.num_points, False, opt.dataset_root, 0.0,
                                                     opt.refine_start)
            elif opt.dataset == 'linemod':
